...
import { createChangeIntentMarkdown } from './lib/deps-release-core.mjs'
...
function main() {
  ...
  if (packageNames.length === 0) {
    console.log('No changed packages, skip creating release intent file')
    return
  }

  const runId = process.env.GITHUB_RUN_ID || `${Date.now()}`
  const releaseFile = `.changeset/packages-deps-${runId}.md`
  const content = createChangeIntentMarkdown(
    packageNames,
