...
- 当同时开启 `vue: true` 时，关闭 `vue/no-deprecated-slot-attribute`、`vue/no-useless-template-attributes` 与 `vue/singleline-html-element-content-newline`，允许小程序原生 slot 投影和编译器专用的 slot 属性；同时对命名为 `id` 或 `class` 的 Vue props 给出 warning，因为小程序 `properties` 可能无法稳定接收到这些值
