...
This repository targets Node `>=22.13.0` and pnpm 11.x. Husky hooks run on `prepare`; install dependencies before committing to ensure hooks exist locally. Use `npx @eslint/config-inspector` or `pnpm build:inspector` when you need a visual diff of rule changes.
